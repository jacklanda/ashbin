...
def pattern():
    return{
            ...
            "phrase_zh": ".*(回复|楼主|管理员|订阅|评论|收藏本页|联系站长|注册|登录|版主|游客|浏览|搜索|下载|首页|版权所有|正在加载|加载中|请稍后|公网安备|首页|上一页|下一页|完善密码).*",
            "key_val": "\w+:\s\".*\",",
            #"multi_space": "([^\u4e00-\u9fa5A-Za-z])\s{0,}([^\u4e00-\u9fa5A-Za-z])", 
            "single_punctuation": "(·|•|【|】|～|~|#|\*|\?|？|《|》|<|>|,|，|。|!|！|@|\$|%|\^|&|\(|\)|（|）|—|\+|=|\|\{|\}|\[|\]|【|】|、|‘|’|\'|\"|\\|/|…|……|[\.]{6}|[\.]{3}|[\.]{2}|×|￥|￥|:]|“|”|_|-)",
            "single_punc": "(，|。|：|；|！|？|“|”|‘|’|～|`|【|】|……|…|×|-|——|—|《|》|（|）|!|\?|\(|\))",
            "except_punc": "[^\u4E00-\u9FA5A-Za-z0-9\n\.\-，,。“”‘’？！]",
            #"digits_with_space": "[+-]?(\s*\d+\s*)*",
            "continue_letter": "[A-Za-z]+",
            "continue_digits": "[0-9]{5,}",
            "only_spaces": "\s{2,}",
            "dot": "\.",
            "bar": "-",
...
def punc2zh():
    return[
            "(\d+.{0,2}(-).{0,2}\d+)+",    # 2000-3000 转换为：2000到3000
            "(\d+.{0,2}(.).{0,2}\d+)+"                     # 3.14 转换为：3点14
            ]
