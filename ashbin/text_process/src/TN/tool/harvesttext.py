...
COM_QUANTIFIERS = '(匹|张|座|回|场|尾|条|个|首|阙|阵|网|炮|顶|丘|棵|只|支|袭|辆|挑|担|颗|壳|窠|曲|墙|群|腔|' \
                  ...
                  '纪|岁|世|更|夜|春|夏|秋|冬|代|伏|辈|丸|泡|粒|颗|幢|堆|条|根|支|道|面|片|张|颗|块|台|米|克|°C)'
...
class NSWNormalizer:
    ...
    def normalize(self):
        ...
        if matchers:
            for matcher in matchers:
                text = text.replace(matcher[1], str(int(matcher[1])), 1)
                text = text.replace(matcher[2], '点', 1)
                text = text.replace(matcher[3], str(int(matcher[3])) + '分', 1)
                if matcher[4] != '':
                    text = text.replace(matcher[4], '', 1)
                    if matcher[5] != '':
                        text = text.replace(matcher[5], str(int(matcher[5])) + '秒', 1)
...
def nsw_test():
    ...
    nsw_test_case('90后')
    nsw_test_case('90年代')
    nsw_test_case('新年快乐。要是少放点鞭炮就好了。小豆豆被12：00的鞭炮炸醒后就坐在那不敢睡了。哎 ')
    nsw_test_case('2014首 ')
    nsw_test_case('其中猪肉价格上涨1102% ')
    nsw_test_case('84875°C ')
